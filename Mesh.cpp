...
void Mesh::CreateBuffers(Vertex* vertData, unsigned int vertNum, unsigned int* indexData, unsigned int indexNum)
{
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		initialVertexData.pSysMem = vertData; // pSysMem = Pointer to System Memory
...
Mesh::Mesh(Vertex* vertData, unsigned int vertNum, unsigned int* indexData, unsigned int indexNum)
{
	vertexCount = vertNum;
	indexCount = indexNum;
	CalculateTangents(vertData, vertNum, indexData, indexNum);
	CreateBuffers(vertData, vertNum, indexData, indexNum);
}
